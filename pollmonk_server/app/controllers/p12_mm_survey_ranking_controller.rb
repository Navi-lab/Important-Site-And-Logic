require 'base64'
class P12MmSurveyRankingController < ApplicationController
	skip_before_action :verify_authenticity_token

	def index
		if params[:survey_id]
			survey_id = Base64.decode64(params[:survey_id])
			@survey_grade = P12MmSurveyRanking.where("survey_id = ?", survey_id).order("survey_q_id, id")
		end

		if params[:survey_q_id]
			survey_id = Base64.decode64(params[:survey_q_id])
			@survey_grade = P12MmSurveyRanking.where("survey_q_id = ?", survey_id)
		end

		if params[:edit_question]
			question_id = Base64.decode64(params[:edit_question])
			@survey_grade = P12MmSurveyRanking.where("survey_q_id = ?", question_id)
		end
		render json: {status: 'SUCCESS', message: 'List of Survey Grade', data: @survey_grade}, status: :ok
	end

	def show
		@survey_grade = P12MmSurveyRanking.find(Base64.decode64(paramsparams[:id]))
		render json: {status: 'SUCCESS', message: 'Survey Grade Details', data: @survey_grade}, status: :ok
	end

	def create
		@survey_grade = P12MmSurveyRanking.new(create_upd_params)
	...
	def batch_create
		survey_answer_params['survey_answers'].each do |answers|
			@survey_grade = P12MmSurveyRanking.new(answers)
			if @survey_grade.save
	...
	def create_ranking_ans
		params[:feedback_file][0]['answer'].each_pair do |key, val|
			@survey_grade = P12MmSurveyRanking.find(key.to_i)
			@survey_grade.answer = val.to_i
			@survey_grade.save
		end
	...
	def update
		@survey_grade = P12MmSurveyRanking.find(params[:id])
	...
	def batch_delete
		survey_delete_params['survey_delete'].each do |id|
			@survey_grade = P12MmSurveyRanking.find(id[:id]);
			if @survey_grade.destroy
	...
	def create_upd_params
		params.permit(:survey_id, :survey_q_id, :answer)
	end

	def survey_answer_params
		params.permit(:survey_answers => [:survey_id, :survey_q_id, :answer_label])
	end
